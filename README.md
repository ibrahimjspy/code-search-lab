...
Use it from a terminal or an agent with shell access. No MCP server is required.
...
working directory**. The CLI builds or refreshes the index automatically.
...
| BM25 **(default)** | Your question shares words with identifiers, paths or source | `python codesearch search "question" --repo path/to/project` |
...
Initial embedding generation takes time. Each fresh CLI process also reloads the
model: **warm retrieval time is not fresh-process latency**. BM25 remains the default.
...
| `--db PATH` | Override the automatically isolated database location |
| `--path src/component/` | Restrict results by source-relative path prefix |
| `--mode bm25\|dense\|hybrid` | Choose the retrieval strategy |
| `--include-docs` | Include document records |
| `--limit 1..20` | Result count; defaults to 5 |
...
CI runs the lightweight tests and lexical evaluation across Windows, macOS and
Linux with Python 3.10 and 3.12; it does not run full embedding inference.
...
- A resident model process to reduce repeated startup cost.
